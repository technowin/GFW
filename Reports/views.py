...
from GFW.settings import MEDIA_ROOT
app = Flask(__name__)
# Create your views here.
...
def preprocess_data_list(result_data):
    ...
    for row in result_data:
        processed_row = []
        for value in row:
            if isinstance(value, str) and 'GFWformfiles_' in value:
                file_links = []
                file_ids = [v.replace('GFWformfiles_', '') for v in value.split(',') if v.startswith('GFWformfiles_')]
